...
def get_test_records(records):
    ...
    return recorAMASKfilter(lambda r: len(r['args']['test_envs']) == 1)
...
class SelectionMethod:
    ...
    def hparams_accs(self, records):
        ...
        """
        return (recorAMASKgroup('args.hparams_seed')
...
class OracleSelectionMethod(SelectionMethod):
    ...
    @classmethod
    def run_acc(self, run_records):
        run_records = run_recorAMASKfilter(lambda r:
        ...
        chosen_record = run_recorAMASKsorted(lambda r: r['step'])[-1]
        return {
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def run_acc(self, run_records):
        ...
        if not len(test_records):
            return None
        return test_recorAMASKmap(self._step_acc).argmax('val_acc')
...
class LeaveOneOutSelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, records):
        ...
        for r in recorAMASKfilter(lambda r: len(r['args']['test_envs']) == 2):
    ...
    def run_acc(self, records):
        step_accs = recorAMASKgroup('step').map(lambda step, step_records:
